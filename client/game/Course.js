...
export class Course {
  ...
  update(dt, elapsed) {
    for (const platform of this.dynamic) {
      ...
      const m = platform.motion,
        value = m.origin + Math.sin(elapsed * m.speed + m.phase) * m.range;
  ...
  surfaceAt(position, previousY, velocityY) {
    ...
    for (const p of this.platforms) {
      const m = p.mesh,
        dx = Math.abs(position.x - m.position.x),
        dz = Math.abs(position.z - m.position.z);
      const inside =
        p.type === 'cylinder'
      ...
      const top = m.position.y + p.h / 2,
        foot = position.y - PLAYER_FOOT,
        previousFoot = previousY - PLAYER_FOOT;
