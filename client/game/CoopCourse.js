...
export class CoopCourse extends CourseBuilder {
  ...
  // `LANE_WIDTH` здесь — не полуширина дорожки, и это НЕ обмолвка.
  //
  // Рядом `LANE_WIDTH / 2` означает половину дорожки, поэтому число так и просится «исправить» на 6.
  // Оно намеренно шире: клиентский счётчик — предсказание, и от него требуется догонять серверный, а
  // не совпадать с ним. Сужать его — единственное направление, которым можно навредить. См.
  // `shared/courseProgress.js` и `server/checkpointFrameDirection.test.mjs`.
  //
  // Форма проверки важнее ширины: «я за чертой» остаётся истинным и на следующих кадрах, поэтому
  // отставание закрывается само. Замена её на проверку ПЕРЕСЕЧЕНИЯ сделала бы отставание вечным.
