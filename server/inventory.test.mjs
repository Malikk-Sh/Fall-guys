...
test('клиент не может экипировать предмет, которого нет в server inventory', () => {
  ...
  assert.equal(inventory.equip(account.id, 'body', 'head-crown').reason, 'wrong-slot');
  db.close();
});
...
test('сервер материализует достижения в cosmetics ownership и проверяет loadout', () => {
  ...
  for (const id of ['body-sunset', 'head-crown', 'trail-spark', 'finish-stars', 'finish-champion']) {
  ...
  assert.equal(inventory.equip(account.id, 'finish', 'finish-champion', 4001).ok, true);
