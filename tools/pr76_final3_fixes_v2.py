...
def replace_in_test(path, test_name, old, new, label):
    file = Path(path)
    text = file.read_text(encoding='utf-8')
    marker = f"test('{test_name}'"
    start = text.find(marker)
    if start < 0:
        raise SystemExit(f'{label}: test marker not found')
    ...
    if count != 1:
        raise SystemExit(f'{label}: expected 1 match in test, found {count}')
    block = block.replace(old, new)
    file.write_text(text[:start] + block + text[end:], encoding='utf-8')


# 1. Guard async incident search results with both view revision and admin session generation.
replace_once(
...
# 2. Record support/forced-logout only when durable/local cleanup completed.
replace_once(
...
success_name = 'support actions enforce split capabilities, revoke every login path and audit mutations'
replace_in_test(
    'server/adminPlayerSupportRoutes.test.mjs', success_name,
...
""",
    'success incident assertion'
)

partial_name = 'support logout reports partial cleanup failures instead of claiming success'
replace_in_test(
    'server/adminPlayerSupportRoutes.test.mjs', partial_name,
    """const { db, adminAuth, app, auth, disconnected, reconnectRevocations } = prepare({
...
""",
    'partial incident assertion'
)

http_name = 'support logout continues local cleanup when HTTP session revocation fails'
replace_in_test(
    'server/adminPlayerSupportRoutes.test.mjs', http_name,
    "const { db, adminAuth, app, auth, disconnected, reconnectRevocations } = prepare();",
    "const { db, adminAuth, app, auth, disconnected, reconnectRevocations, incidentEvents } = prepare();",
...
# 3. Protect requested restore source before starting a backup service that applies retention.
replace_once(
    'deploy/restore.sh',
