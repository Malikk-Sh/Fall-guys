...
export function buildPortal({ platform = 'yandex', outDir } = {}) {
  ...
  // Политика конфиденциальности на площадке — СВОЯ, и подменяется она здесь.
  //
  // Полная политика описывает аккаунты, вход через Google и обработку технических данных нашим
  // сервером. В портальной сборке всё это неправда: аккаунтов нет, входа нет, до сервера не доходит
  // ни один запрос — последнее проверено замером в `e2e/portal.spec.js`, а не выведено. Уехать
  // такой странице значило бы заявить игроку обработку, которой не происходит.
  //
  // Короткая версия лежит вне `client/` намеренно: на нашем сайте она не нужна и не должна быть
  // доступна. Подмена, а не исключение: ссылка на `./privacy/` стоит в разметке меню, и без файла
  // она вела бы в 404.
  const portalPrivacy = path.join(ROOT, 'tools', 'portalPrivacy.html');
  if (!fs.existsSync(portalPrivacy)) {
    throw new Error('нет tools/portalPrivacy.html — на площадку уехала бы политика нашего сайта');
  }
  fs.copyFileSync(portalPrivacy, path.join(out, 'privacy', 'index.html'));
