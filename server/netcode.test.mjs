...
test('лежащий сбитый игрок не проваливается, а сбитый в полёте — падает', () => {
  // Сбивание едет по сети отдельным состоянием, и оно перекрывает 'ground' в снапшоте: у лежащего
  // игрока state === 'knockdown', хотя он на опоре. Раньше экстраполяция считала такого летящим и
  // применяла к нему гравитацию — на потолке в 250 мс это 0.7 единицы вниз и рывок обратно на
  // следующем пакете. У себя сбитый лежал ровно, поэтому увидеть это можно было только на ЧУЖОМ
  // экране, то есть ровно там, ради чего состояние и передаётся.
  const lying = new SnapshotBuffer();
  lying.push(1000, [{ id: 'a', x: 0, y: 0.88, z: 0, ry: 0, vx: 3, vy: 0, vz: 0, state: 'knockdown' }]);
  const far = lying.sample('a', 1250);
  assert.ok(far.extrapolated, 'случай именно про экстраполяцию, а не про интерполяцию');
  assert.equal(far.y, 0.88, 'лежащий остаётся на опоре');
  // Горизонталь при этом достраивается как раньше: инерция сбитого игрока никуда не делась.
  assert.ok(Math.abs(far.x - 3 * 0.25) < 1e-9);

  // Сбитый в полёте — сшибли с моста — остаётся в том же состоянии, но падать обязан.
  const falling = new SnapshotBuffer();
  falling.push(1000, [{ id: 'a', x: 0, y: 6, z: 0, ry: 0, vx: 0, vy: -4, vz: 0, state: 'knockdown' }]);
  assert.ok(falling.sample('a', 1200).y < 6, 'сбитого в воздухе гравитация по-прежнему тянет вниз');
});
