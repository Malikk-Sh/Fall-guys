...
export class Effects {
  constructor(scene, quality = 'high') {
    ...
    this.items = [];
    this.pool = [];
    this.max = quality === 'low' ? 34 : 72;
    this.geometry = new THREE.IcosahedronGeometry(0.085, 0);
    this.materials = new Map();
  }
  material(color) {
    if (!this.materials.has(color))
      this.materials.set(color, new THREE.MeshBasicMaterial({ color, transparent: true }));
    return this.materials.get(color);
  }
  burst(position, color = 0xffffff, count = 10, power = 1) {
    const total = Math.min(count, this.max - this.items.length);
    for (let i = 0; i < total; i++) {
      const mesh = this.pool.pop() || new THREE.Mesh(this.geometry, this.material(color));
  ...
  update(dt) {
    for (let i = this.items.length - 1; i >= 0; i--) {
      ...
      mesh.material.opacity = Math.max(0.05, t);
    }
  }
  clear() {
