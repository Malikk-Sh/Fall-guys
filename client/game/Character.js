...
export class Character {
  ...
  animate(dt, { speed = 0, grounded = true, vertical = 0, diving = false } = {}) {
    ...
    // Эмоция прерывается сменой состояния движения: прыжок и подкат важнее позы, а поза,
    // пережившая прыжок, выглядела бы как застрявшая анимация.
    if (this.cosmetics?.emoteActive && (this.state === 'air' || this.state === 'dive')) {
