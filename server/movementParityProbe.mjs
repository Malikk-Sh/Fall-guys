...
// Один сетевой отсчёт на два кадра физики: 60 Гц против 30 Гц, как у живого клиента.
...
function runRace(runtime, { seed, difficulty, skill, index }) {
  ...
  const player = { bot: false, checkpoint: 0, finished: false, last: bot.snapshot() };
  const room = {
    mode: GAME_MODE.RACE,
...
function main() {
  ...
  const lines = [
    ...
    `согласие по опоре     ${pct(metrics.agreementRate)}   (порог ≥ ${pct(policy.minGroundAgreementRate)})`,
    `  расхождений         ${metrics.groundedMismatch}`,
    `  сервер дал пол      ${metrics.shadowGroundedOnly}   (порог ${policy.maxShadowGroundedOnlySamples})`,
    ...
    `траектория      сред. ${metrics.freeTrajectoryError.mean.toFixed(4)}   (порог ≤ ${policy.maxFreeTrajectoryErrorMean})`,
    `                 p95  ${metrics.freeTrajectoryError.p95.toFixed(4)}`,
    `                 макс ${metrics.freeTrajectoryError.max.toFixed(4)}   (порог ≤ ${policy.maxFreeTrajectoryErrorMax})`,
