...
const { VerifiedLeaderboard, courseKey, VERIFICATION_VERSION } = require('./verifiedLeaderboard');
...
test('без идентификатора игроки не склеиваются в одну строку', () => {
  ...
  assert.equal(board.get(9, 'normal').length, 2);
  board.close();
});
...
test('своё место считается и за пределами показанной десятки', () => {
  ...
  const top = board.get(4, 'normal', 3, 'p11');
  ...
  const standing = board.standing(4, 'normal', 'p11');
  ...
  const leader = board.standing(4, 'normal', 'p0');
  ...
  assert.equal(board.standing(4, 'normal', 'кого-тут-нет'), null);
  board.close();
});
...
test('своя строка помечается признаком self', () => {
  ...
  assert.deepEqual(
    board.get(6, 'normal', 10, 'мой').map(entry => [entry.name, entry.self]),
    [
      ['Не я', false],
      ['Я', true]
    ]
...
test('рекорды переживают перезапуск процесса', () => {
  ...
  try {
    ...
    assert.deepEqual(
      after.get(42, 'normal').map(entry => [entry.name, entry.time]),
      [['Рекордсмен', 15_000]],
...
test('версия алгоритма проверки хранится вместе с рекордом', () => {
  ...
  assert.equal(board.get(8, 'normal')[0].verificationVersion, 4);
  assert.equal(board.standing(8, 'normal', 'p1').verificationVersion, 4);
  board.close();
});
...
test('таблица не растёт бесконечно', () => {
  ...
  assert.deepEqual(board.get(1, 'normal'), [], 'самая давняя трасса вытеснена целиком');
  assert.equal(board.get(3, 'normal').length, 1);
  ...
  assert.equal(board.get(3, 'normal', 25).length, 2, 'на трассу хранится не больше заданного');
  board.close();
});
