...
//
// Поэтому падение этого теста означает поломку матча, а не «водитель не справился с трассой».

import { test, expect } from '@playwright/test';

// Запас на прохождение. Замер этого водителя на этой трассе — худшее 20 секунд игрового времени из
// 112 вариантов тайминга, — но потолок взят кратно больше: падение возвращает на чекпоинт и сегмент
// приходится проходить заново, а физика ограничена пятью подшагами на кадр, то есть при низком FPS
// раннера забег растягивается по настенным часам пропорционально.
...
// «вперёд», подруливать к оси трассы и жать прыжок. Одного «вперёд» не хватает: финальный сегмент
// подобранной трассы ветреный, и на нём боком сдувает с платформы. Всё остальное — разбор
// препятствий, выбор момента, реакция на удар — сюда не добавляется сознательно: умный водитель
// становится второй игровой логикой, которую придётся чинить при каждой правке баланса, и падение
// теста перестаёт что-либо означать. Трасса подобрана под ЭТОГО водителя, а не наоборот.
//
