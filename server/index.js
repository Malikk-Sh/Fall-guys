...
app.get('/health', (_req, res) =>
  res.json({ ok: true, service: 'wobble-rush-3d', rooms: rooms.size, version: '2.0.0' })
);
...
const broadcast = (room, data) => {
  for (const player of room.players.values()) send(player.ws, data);
};
const roomCode = () => {
  let value;
  do
...
const publicPlayer = ({ id, name, ready, finished, time, rematch, color }) => ({
  id,
  name,
  ready: !!ready,
  finished: !!finished,
...
function leave(ws) {
  ...
  if (!room.players.size) rooms.delete(room.code);
    ...
    if (room.host === ws.id) room.host = room.players.keys().next().value;
    emitLobby(room);
  }
}
...
wss.on('connection', ws => {
  ...
  ws.isAlive = true;
  send(ws, { type: 'hello', id: ws.id, serverTime: Date.now() });
  ws.on('pong', () => {
    ws.isAlive = true;
  });
  ws.on('message', raw => {
    ...
    try {
      ...
      return send(ws, { type: 'error', message: 'Malformed multiplayer message.' });
    }
    ...
    if (message.type === 'create') {
      ...
      if (rooms.size >= MAX_ROOMS)
        return send(ws, { type: 'error', message: 'The party service is full. Try again soon.' });
      const code = roomCode(),
    ...
    if (message.type === 'join') {
      ...
      if (!room) return send(ws, { type: 'error', message: 'Room not found. Check the four-letter code.' });
      if (room.started) return send(ws, { type: 'error', message: 'That race has already started.' });
      if (room.players.size >= MAX_PLAYERS)
        return send(ws, { type: 'error', message: 'That party is full (16 players).' });
      return addPlayer(room, ws, message.name);
    }
    const room = rooms.get(ws.room),
      player = room?.players.get(ws.id);
    if (!room || !player) return send(ws, { type: 'error', message: 'Create or join a room first.' });
    room.updatedAt = Date.now();
    ...
    if (message.type === 'start') {
      if (room.host !== ws.id)
        return send(ws, { type: 'error', message: 'Only the host can start the race.' });
      if (room.started) return;
      if (!room.players.size || ![...room.players.values()].every(item => item.ready))
        return send(ws, { type: 'error', message: 'Every runner must be ready.' });
      room.started = true;
...
if (require.main === module)
  server.listen(port, () => console.log(`Wobble Rush 3D v2 on http://localhost:${port}`));
