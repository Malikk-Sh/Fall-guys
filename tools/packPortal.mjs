...
// ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ДАТА СТАНОВИТСЯ ПОЛЯМИ ZIP, — и потому единственное, где она проверяется.
//
// Проверку я ставил трижды и трижды не туда: сначала её не было вовсе, потом она встала на разбор
// `SOURCE_DATE_EPOCH` — то есть на границу ВВОДА, а не ИСПОЛЬЗОВАНИЯ. Обход остался очевидный:
// `makeZip(entries, { date })` — экспортированный вход, мимо всякого разбора. Замерено на том коде:
// `new Date(0)` давал в архиве 2098-01-01, 2200 год — 2072-й, а `Invalid Date` — месяц 0 и день 0,
// то есть заведомо негодную запись.
//
// Поэтому проверка переехала сюда. Здесь её не обойти ни одним путём: и умолчание, и переменная
// окружения, и прямой вызов приходят в `dosStamp` одинаково.
//
// Поля при этом читаются UTC-геттерами, а не локальными: с локальными один и тот же
// `SOURCE_DATE_EPOCH` давал на разных машинах разные заголовки — три пояса, три суммы SHA-256, а в
// Токио разъезжалась и дата. Воспроизводимость держалась на одной машине и ломалась там, где нужна.
// Наружу не экспортируется намеренно: отдельный доступ к проверке — это приглашение вызвать её
// «где-нибудь ещё», а именно так вопрос и разъехался по двум местам в прошлый раз.
function assertZipDate(date) {
  if (!(date instanceof Date) || !Number.isFinite(date.getTime())) {
    throw new Error(`метка времени архива не является датой: ${String(date)}`);
  }

  const year = date.getUTCFullYear();
  if (year < 1980 || year > ZIP_LAST_YEAR) {
    throw new Error(
      `метка времени архива — ${date.toISOString()}; формат ZIP хранит только 1980–${ZIP_LAST_YEAR}`
    );
  }
  return date;
}
...
function dosStamp(date) {
  const year = assertZipDate(date).getUTCFullYear();

  const time =
    ((date.getUTCHours() << 11) | (date.getUTCMinutes() << 5) | (date.getUTCSeconds() >> 1)) & 0xffff;
  const day = (((year - 1980) << 9) | ((date.getUTCMonth() + 1) << 5) | date.getUTCDate()) & 0xffff;
  return { time, day };
}
...
// Разбор `SOURCE_DATE_EPOCH` — и ТОЛЬКО разбор.
//
// Годность самой даты проверяет `dosStamp`, потому что туда приходят все пути, а сюда — один.
// Здесь остаётся то, чего `dosStamp` знать не может: что строку вообще просили прочесть как число
// секунд. `SOURCE_DATE_EPOCH=2026-01-01` разбирается `parseInt` как 2026 секунд, то есть как 1970
// год, — ошибка правдоподобная, и молчаливый откат к эпохе по умолчанию скрыл бы её.
//
// Пустое или не заданное значение — другое дело: это не «просили и не смогли», а «не просили», и
// тогда берётся начало эпохи ZIP. Постоянная метка нужна ради воспроизводимости: с `new Date()` тот
// же билд давал архив того же размера, но другими байтами.
...
export function packPortal({ platform = 'yandex', outFile } = {}) {
  // Метка разбирается здесь, а ПРОВЕРЯЕТСЯ одним заслоном в `dosStamp` — второй проверки тут нет
  // намеренно.
  //
  // Сначала я поставил её и сюда, «чтобы негодное значение не унесло удачный билд». Проверил — и
  // довод оказался ложным: `buildPortal` дерево пересобирает, сборка успешна, так что после отказа
  // на метке `dist/yandex` остаётся годным. Терялись бы только доли секунды работы сборки.
  //
  // А цена второй проверки не нулевая, и именно она привела сюда: разложив один и тот же вопрос по
  // двум местам, я трижды чинил не то место. Заслон один, стоит там, куда приходят все пути.
