...
// Прямая по Z с заданной скоростью: путь и смещение совпадают.
function straight(speed, count) {
  const step = (speed * SEND_MS) / 1000;
  return Array.from({ length: count }, (_, i) => ({ x: 0, y: 1, z: -i * step }));
}
...
test('пик остаётся максимальным за забег, а не последним', () => {
  const { player } = feed([...straight(9, 30), ...straight(2, 30)]);
  assert.ok(player.sustainedSpeedPeak.average > 6, 'быстрый участок обязан пережить медленный');
});
