...
//
// Инвариант теперь есть, и держит его не этот файл, а `server/testing/duplicateTestReporter.mjs`:
// он считает, сколько раз тест ВЫПОЛНИЛСЯ, и потому ловит любой способ затащить файл, включая те,
// которых регулярка не видит принципиально. Здесь остаётся быстрое приближение: оно работает без
// прогона всего набора и называет виновную пару файлов, что при разборе полезнее, чем «этот тест
// выполнился дважды». Авторитет — у репортёра; расхождение между ними следует читать в его пользу.
