...
### За обратным прокси (Nginx на своём сервере)

Сервер ограничивает частоту по адресу клиента: 24 одновременных сокета и 40 операций с комнатами в
минуту. За прокси `remoteAddress` — это адрес самого Nginx, один на всех, поэтому лимиты начинают
делиться между всеми игроками сразу и отказы сыплются на случайных людей.

Установите `TRUST_PROXY=1`, и сервер будет брать адрес из `X-Forwarded-For`. **Включайте это только
если Node недоступен из интернета напрямую** — иначе заголовок подделает кто угодно и обойдёт лимиты.
...
location /ws {
    ...
    # Больше, чем период heartbeat (15 с), иначе прокси будет рвать живые соединения.
