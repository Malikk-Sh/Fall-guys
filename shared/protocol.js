...
export const MESSAGE_SCHEMAS = Object.freeze({
  ...
  [C2S.PLAYER_STATE]: {
    ...
    // Время трассы, по которому клиент снял это состояние, — то же число, что он передаёт в
    // `Course.update` и `Course.interact` (секунды с начала забега).
    //
    // Нужно оно ровно для одного: сверить опору под игроком с ПОДВИЖНОЙ платформой. Момент приёма
    // пакета для этого не годится — за сетевую задержку платформа уезжает, и сравнение старой
    // позиции игрока с новой позицией опоры даёт расхождение из ничего. Без этого поля такие
    // выборки приходилось исключать из доказательств целиком.
    //
    // Поле необязательное намеренно: его отсутствие возвращает прежнее поведение (подвижные опоры
    // не сверяются), а не ломает разбор. Авторитетом оно не становится — по нему только выбирается
    // момент, на который смотреть, и проверка диапазона у него своя.
    courseTime: optional(num(0, 86_400))
  },
