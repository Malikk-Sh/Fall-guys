...
### Правило прогресса — одно на обе стороны

Чекпоинт и финиш — граница результата, и решает её сервер. Но правило, по которому арка считается
пройденной, обязано быть у клиента и сервера **буквально одно**: `crossedCheckpoint` из
`shared/courseProgress.js`. Две отдельные реализации одного правила уже расходились в проде, и
расхождение стоило дорого — сервер отменял игроку честно пройденную арку, а забег, в котором клиент
считал трассу пройденной, а сервер нет, не заканчивался вовсе.

Поэтому при правке прогресса:

- не заводите второе место, которое знает про рамки арки или про пересечение её плоскости;
...
- «оказался за чертой» и «пересёк черту» — разные условия, и подменять второе первым нельзя:
  пересечение бывает один раз, а нахождение за чертой — сколько угодно.

Более строгий коридорный вариант того же правила (`server/raceProgressSpatialGuard.js`) живёт
отдельно и относится к shadow-пути; он пользуется той же геометрией пересечения из `shared/`.
