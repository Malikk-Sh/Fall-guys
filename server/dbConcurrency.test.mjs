...
test('запись из игры дожидается чужой транзакции вместо немедленной ошибки', async () => {
  ...
  try {
    ...
    // Подписка на выход оформляется здесь, а не в finally.
    //
    // ChildProcess отдаёт 'exit' ровно один раз и не переигрывает его для тех, кто подписался
    // позже. Дочерний процесс выходит сразу после COMMIT — то есть примерно тогда же, когда
    // родитель заканчивает своё ожидание, — поэтому подписка в finally успевала бы к разбору:
    // промис не разрешился бы никогда и подвесил весь прогон тестов. Ранняя подписка снимает
    // вопрос целиком, в том числе для случая, когда ребёнок падает, не дойдя до маркера.
