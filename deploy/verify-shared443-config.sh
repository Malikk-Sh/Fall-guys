...
NGINX
  ...
  # Regression for the production failure seen behind shared-443:
  # 1) /admin must return one relative redirect, never an absolute URL containing the internal TLS port;
  # 2) canonical /admin/ must bypass Express' non-strict /admin redirect route and fetch index.html once.
