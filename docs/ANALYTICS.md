...
`sustained_speed_peak` и `sustained_speed_net_at_peak` — две меры ОДНОЙ величины на одном и том же
двухсекундном окне, записанные в сотых долях (делите `total / samples` на 100). Первая — среднее по
пакетам, ровно то, что принимает решение о признаке `sustained-speed`. Вторая — чистое смещение за
то же окно, как это считает кооператив. `detail` — `когорта:состояние`, где когорта отделяет забеги, в которых признак `sustained-speed`
сработал (`noted`), израсходовал запас (`over`) и не срабатывал вовсе (`quiet`). Без этого деления
среднее считалось бы по всему населению, где спокойных забегов заведомо больше, и они размыли бы
ровно те значения, ради которых замер сделан: сшить эти строки с `movement_anomaly` нельзя —
`GameplayMetrics` хранит только суммы по ключу измерений.

Расходятся они ровно тогда, когда путь длиннее прямой между концами окна. Замер отвечает на вопрос,
который иначе решался бы наугад: если смещение сильно ниже среднего, у ложных срабатываний виновата
формула; если они близко — игрок действительно ехал быстро, и разговор про порог. Только гонка:
у кооператива своя проверка, уже считающая смещением.
