...
//
// ЧЕГО ЭТА ПРОВЕРКА НЕ ДЕЛАЕТ. Она читает исходники, а не наблюдает выполнение, поэтому «выполняется
// ровно один раз» она не доказывает: путь, собранный в рантайме, или реэкспорт через файл, который
// сам не тест, она не увидит. Это сеть на те написания, которые пишут руками, — и четыре круга
// ревью подряд находили дыру именно в сети, а не в коде, который она сторожит.
//
// Строго сильнее была бы проверка по НАБЛЮДАЕМОМУ выполнению: одно и то же имя теста, пришедшее из
// двух файлов, есть удвоение независимо от того, как оно записано в исходнике. Здесь этого нет, и
// выдавать одно за другое не нужно.
...
function importedTestFiles(file) {
  ...
  //
  // `require` в том же ряду: набор смешанный — `integration.test.js` написан на CommonJS, и
  // `testFilesOnDisk()` намеренно включает `.test.js`. Требуемый файл регистрирует свои тесты в
  // процессе требующего и выполняется ещё раз, когда его запускают отдельно. Удвоение то же,
  // запись другая.
  const pattern =
    /(?:^|\s)(?:(?:import|export)\s*(?:[\w*{},\s]*?\s*from\s*)?|require\s*)[('\s]*['"]\.\/([\w.-]+\.test\.m?js)['"]/gm;
  return [...source.matchAll(pattern)].map(match => `server/${match[1]}`);
}

test('тест не запускает другой тест ни одной из написанных руками форм', () => {
  ...
  //
  // Правило теперь одно: запуск задаёт manifest, и только он. Это приближает «перечислен ровно один
  // раз» к «выполняется ровно один раз», но не делает вторым первое: см. оговорку в шапке файла.
  const offenders = [];
